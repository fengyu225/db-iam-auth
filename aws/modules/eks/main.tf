...
resource "aws_security_group_rule" "nodes_nlb_ingress" {
  type              = "ingress"
  from_port         = 80
  to_port           = 65535
  protocol          = "tcp"
...
resource "aws_iam_role_policy" "nodes_oidc_policy" {
  name = "eks-node-oidc-policy"
...
resource "kubernetes_config_map" "aws_auth" {
  ...
  data = {
    mapRoles = yamlencode([
      ...
      {
        rolearn  = "arn:aws:iam::072422391281:role/aws-reserved/sso.amazonaws.com/AWSReservedSSO_AdministratorAccess_a6294736edac8b8f"
        username = "admin"
